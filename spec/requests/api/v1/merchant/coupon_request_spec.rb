...
RSpec.describe 'Coupon endpoints' do
  ...
  it 'returns a specific coupon with usage count' do
    ...
    attributes = dta[:attributes]

    expect(attributes[:name]).to eq('Buy One Get One 50')
    exepct(attributes[:code]).to eq(@coupon.code)
    exepct(attributes[:discount_type]).to eq(@coupon.discount_type)
    exepct(attributes[:discount_value]).to eq(@coupon.discount_value)
